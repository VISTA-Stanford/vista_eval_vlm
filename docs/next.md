...
- **rung-0 PROCEEDING — 0a accepted (declared delta); feb26 CT-load validated; 0b weighted BACKLOGGED to GPU box (2026-07-08) → [docs/vm-status/2026-07-08-rung0-reproduce-ryan-feb26.md](vm-status/2026-07-08-rung0-reproduce-ryan-feb26.md)** — VM `phil-sllm-01` ran the weight-free 0a preflight: `nifti_path` shape clean/single (OQ-R4: all 2,362 v1_1 PFS CT rows `/mnt/…/nov25/<study>__<series>.nii.gz` → resolver reconstructs to feb26, 0 bad); **feb26 blob-existence = 86.9% (309/2,362 missing)** — the v1_1↔v1_5 CT-set drift. **Phil accepted 86.9% as a declared delta** (class-2, not a handback): stratify 0c to `used_image==1` rows for a like cohort. **feb26 CT-load then validated end-to-end weight-free** via the golden harness (`axial_all_image --limit 30`: 14/14 CT-present rows load 30 slices, hashes+selection consistent, `source=gcs` throughout) → also **banks the partial `axial_all_image` golden** the prior round couldn't (nov25 was gone). Overlay authored: `configs/all_tasks.rung0.vm.yaml` (uncommitted, gitignored; feb26 prefix + force-GCS + constrained decoding; `subsample:false`+`valid_tasks.json` declared deltas). **next: run the WEIGHTED 0b on the GPU machine** (`python -m vista_run.run_bq --config ../configs/all_tasks.rung0.vm.yaml` — CPU-only here, no GPU) → then 0c stratified comparison. Turnkey 0b/0c commands in the vm-status UPDATE block. This VM changed no code; edits = vm-status doc + this file.
