...
2. **Timeline CSVs** — Local CSVs that contain **patient_string** (or **patient_timeline**): one row per patient with clinical-event text. These are merged with BigQuery rows on `person_id` and still gate which patients enter a task's cohort. They may also include `report` (radiology report) and `embed_time`. For `no_image`/`axial_all_image`/`no_report`/`timeline_only`/`report`, the timeline **text** itself is no longer taken from this CSV column — it's overwritten at prompt-build time by parsing each patient's real **LUMIA `.xml`** file (from `retrieval.corpus_dir`, same directory used by retrieval — see [05-retrieval.md §1](05-retrieval.md)), windowed to **24 months before `embed_time`** by default for `no_image`/`axial_all_image` (matching the scope the legacy `patient_string` CSV was itself generated with, not full unrestricted history; override via the `ehr_timeline_window_before` config key) and to 6 months for `no_report`/`timeline_only`/`report`, fail-closed: a patient with no LUMIA file is dropped from that experiment. See [04-running-the-pipeline.md](04-running-the-pipeline.md#ehr-source-step-5-live-lumia-render-vs-pre-rendered-patient_string) for the full mechanism.
